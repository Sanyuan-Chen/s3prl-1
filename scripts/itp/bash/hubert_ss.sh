...
#save_path=/datablob/users/v-sanych/s3prl_models/hubert/ss/bs${bs}_lr${lr}_acc${acc}_node${node}
#model_path=/datablob/users/v-sanych/pretrained_models/hubert_base_ls960.pt.new

save_path=/datablob/users/v-sanych/s3prl_models/hubert_large/ss/bs${bs}_lr${lr}_acc${acc}_node${node}
...
python3 -m torch.distributed.launch --nproc_per_node ${node} run_downstream.py  \
  -n hubert/ss/bs${bs}_lr${lr}_acc${acc}_node${node}  \
  -p ${save_path}  \
