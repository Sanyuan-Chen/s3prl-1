...
save_path=/datablob/users/v-sanych/s3prl_models/${model_name}/se/bs${bs}_lr${lr}_acc${acc}_node${node}_win_length${win_length}
...
python3 -m torch.distributed.launch --nproc_per_node ${node} run_downstream.py  \
  -n ${model_name}/se/bs${bs}_lr${lr}_acc${acc}_node${node}_win_length${win_length}  \
  -p ${save_path}  \
